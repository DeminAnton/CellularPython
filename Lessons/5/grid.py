...
class GameGrid:
    ...
    def neighbors(self, cell: Cell, region: int = 1) -> list:
        ...
        neighbors_coords_row = [
            row + i
            for i in range(-region, region + 1)
            if 0 <= row + i < Config.grid_rows
        ]
        neighbors_coords_colomns = [
            column + i
            for i in range(-region, region + 1)
            if 0 <= column + i < Config.grid_columns
        ]
    ...
    def step(self):
        new_grid = [
            [Cell(1, (j, col)) for col in range(Config.grid_columns)]
